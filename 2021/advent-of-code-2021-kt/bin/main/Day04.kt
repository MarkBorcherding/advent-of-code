object Day04 {
    ...
    data class GameState(val remainingNumbers: List<Int>, val boards: List<Board> = emptyList(), val pickedNumbers: List<Int> = emptyList(), val win: Win? = null) {
        ...
        fun winningBoard(): Win {
            ...
            val nextBoard =
                if (newWinner != null) {
                    val remainingBoards = this.boards.filterNot { it == newWinner }
                    this.copy(boards = remainingBoards,  win = Win(newWinner, pickedNumbers.toList()))
                } else {
                    this
                }
    ...
    data class Board(val input:IntArray, val size: Int = 5) {
        ...
        fun isWinner(pickedNumbers: List<Int>) = winningNumbers.any { pickedNumbers.containsAll(it)}
