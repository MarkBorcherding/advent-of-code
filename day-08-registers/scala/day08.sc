object Day08 {
  ...
  val sampleInput = """b inc 5 if a > 1
                      |a inc 1 if b < 5
                      |c dec -10 if a >= 1
                      |c inc -20 if c == 10""".stripMargin

val s = """lp dec 419 if mxn >= 7
          |w inc -592 if icg >= -9
          |a dec -29 if q <= 9
          |kt dec 486 if ms != 8
          ...
          |cl dec -167 if gi == 1209""".stripMargin

  case class Instruction(
    register: String,
    operation: Operation,
    condition: Condition) {
    def applyTo(registers: Registers): Registers = {
      if(condition.metBy(registers)) {
        val n = operation.execute(registers.get(register).getOrElse(0))
  ...
  object Operation {
    def apply(operation: String, operand: String): Operation = {
      operation match {
        case "inc" => Increment(operand.toInt)
        case "dec" => Decrement(operand.toInt)
      }
    }
  }
  case class Increment(operand: Int) extends Operation {
    def execute(i: Int): Int = i + operand
  }
  case class Decrement(operand: Int) extends Operation {
    def execute(i: Int): Int = i - operand
  }

  abstract class Condition(register: String, value: Int) {
    val predicate:((Int, Int) => Boolean)

    def metBy(registers: Registers): Boolean =
      predicate(registers.get(register).getOrElse(0), value)
  }

  object Condition {
    def apply(register: String, conditional: String, value: String): Condition = {
      val v = value.toInt
      conditional match {
        case "==" => Eql(register, v)
        case "!=" => NotEql(register, v)
        case ">"  => Gt(register, v)
        case ">=" => GtOrEql(register, v)
        case "<"  => Lt(register, v)
        case "<=" => LtOrEql(register, v)
      }

    }
  }

  case class Eql(register: String, value: Int) extends Condition(register, value)  {
    val predicate = (_:Int).== (_:Int)
  }
  case class NotEql(register: String, value: Int) extends Condition(register, value) {
    val predicate = (_:Int).!=(_:Int)
  }
  case class Lt(register: String, value: Int) extends Condition(register, value) {
    val predicate = (_:Int).<(_:Int)
  }
  case class LtOrEql(register: String, value: Int) extends Condition(register, value) {
    val predicate = (_:Int).<=(_:Int)
  }
  case class Gt(register: String, value: Int) extends Condition(register, value) {
    val predicate = (_:Int).>(_:Int)
  }
  case class GtOrEql(register: String, value: Int) extends Condition(register, value) {
   val predicate = (_:Int).>=(_:Int) 
  }
  ...
  def parse(input: String): Seq[Instruction] =
    input
      .split("\n")
      .map {
        case instruction(r, op, opn, cr, co, cv) => Instruction(r, Operation(op, opn), Condition(cr,co,cv))}

  type Registers = Map[String, Int]

  def run(instructions: Seq[Instruction], registers: Registers): (Registers,Int) =
    instructions
      .foldLeft(registers -> 0){ (reg, ins) =>
        val r = ins.applyTo(reg._1)
        r -> Math.max(reg._2, if(r.values.size > 0)r.values.max else 0)
      }
