...
class ConditionRecord:
    ...
    def __init__(self, row: str):
        ...
        contiguous_blocks = ",".join([contiguous_blocks]*5).strip()
        operational_status = "?".join([operational_status]*5)
...
with Progress() as progress:
    ...
    for row in rows:
        progress.update(task, description=f"Starting {row}", advance=.5)
        condition_records.append(ConditionRecord(row))
        progress.update(task, description=f"Finishing {row}", advance=.5)
